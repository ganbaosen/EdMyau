...
public class InvManager extends Module {
    ...
    private boolean performInventoryAction() {
        ...
        int preferredPearlHotbarSlot = this.pearlSlot.getValue() - 1;
        int inventoryPearlSlot = ItemUtil.findInventorySlot(preferredPearlHotbarSlot, ItemUtil.ItemType.Pearl);
                ...
                if (equippedSlot != -1 || inventorySlot != -1) {
                    ...
                    if (equippedSlot != playerArmorSlot && inventorySlot != playerArmorSlot) {
                        if (mc.thePlayer.inventory.getStackInSlot(playerArmorSlot) != null) {
                            int armorContainerSlot = this.convertSlotIndex(playerArmorSlot);
                            if (mc.thePlayer.inventory.getFirstEmptyStack() != -1) {
                                this.clickSlot(mc.thePlayer.inventoryContainer.windowId, armorContainerSlot, 0, 1);
                            } else {
                                this.clickSlot(mc.thePlayer.inventoryContainer.windowId, armorContainerSlot, 0, 4);
                            }
                        } else {
        ...
        if (preferredPearlHotbarSlot >= 0 && preferredPearlHotbarSlot <= 8 && !usedHotbarSlots.contains(preferredPearlHotbarSlot) && inventoryPearlSlot != -1) {
            usedHotbarSlots.add(preferredPearlHotbarSlot);
            if (inventoryPearlSlot != preferredPearlHotbarSlot) {
                this.clickSlot(mc.thePlayer.inventoryContainer.windowId, this.convertSlotIndex(inventoryPearlSlot), preferredPearlHotbarSlot, 2);
                return true;
            }
        }
            ...
            for (int i = 0; i < 36; i++) {
                if (!equippedArmorSlots.contains(i)
                    ...
                    if (stack != null) {
                        ...
                        boolean isArmorTrash = stack.getItem() instanceof ItemArmor;
                        if (isBlock) {
                        ...
                        if (isArmorTrash || ItemUtil.isNotSpecialItem(stack) && (isBlock && currentBlockCount >= this.blocks.getValue() || isProjectile && currentProjectileCount >= this.projectiles.getValue())) {
                            this.clickSlot(mc.thePlayer.inventoryContainer.windowId, this.convertSlotIndex(i), 0, 4);
                            return true;
                        }
                    }
                }
                ...
                if (this.oDelay <= 0 && this.actionDelay <= 0) {
                    if (this.isEnabled() && this.isValidGameMode()) {
                        if (this.mode.getValue() == 1) {
                            while (this.performInventoryAction()) {
                            }
